...
public class CardAnimator{
    ...
    public CardAnimator(ArrayList<View> viewCollection, int backgroundColor, Context context) {
        mCardCollection = viewCollection;
        mBackgroundColor = backgroundColor;
        mContext = context;
        setup();
    ...
    public void initLayout(){
        ...
        for(View v : mCardCollection){
            ...
            int scaleVal = -(size - index - 1) * 5;
            //scale(v, scaleVal); // origin code
            scale(v, scaleVal, size - index - 2, mContext); // modified by dnbb - jangyoun

            int moveVal = index * mStackMargin;
            //move(v, moveVal, 0); // origin code
            move(v, moveVal, 0, size - index - 2, mContext); // modified by dnbb - jangyoun
            v.setRotation(0);
        }
    }
